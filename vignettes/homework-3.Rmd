...
And then, let's see how the function performs under different bandwiths h. Here, we use normal distribution N(0,1) as as example.
...
## P120 Q6
The KKT condition for glmnet when $0<\alpha<=1$ is $\frac{1}{n}\sum_{i=1}^n(x_{il}(y_i-\sum_{j=1}^px_{ij}\hat{b_j}))-\lambda(1-\alpha)\beta_l=\lambda\alpha s_l$  
...
The function "lasso_reg_with_screening" shown below accepts a new lambda, a design matrix X, a response vector y, an current coefficient vector b, the penalty term lambda, and alpha, and it returns a logical vector indicating all the current non-zero coefficients which violate the KKT condition.
